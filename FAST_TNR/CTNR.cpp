...
void elapsed_time(CString & msg, LARGE_INTEGER & sPos, bool print)
{
	...
	if (print == true)
	{
		...
		str.Format(_T("%s : %.2f s %d %d %d \n"), \
			msg.GetBuffer(), (float)ms_interval / (float)1000, tnr.get_m_map_qry().size(), tnr.get_m_map_qry_seg().size(), g_num_kfn_queries);
...
void CTNR::Get_Params_from_File()
{
	...
	FileName.Format(_T("Baseline_%s_%s_%d_%d.txt"), m_query_fname[0].GetBuffer(), m_data_fname[0].GetBuffer(), rand() % 1000, t.GetSecond() + t.GetMinute() + t.GetHour());
...
void CTNR::_Read_map_file()
{
	...
	//map<pair <pair<int, int>, int> non_dup_edges;

	map<pair<int, int>,int> non_dup_edges;
	...
	while (edge_file.ReadString(strline))
	{
		...
		non_dup_edges.insert(make_pair(make_pair(edge.node1, edge.node2), edge.edgeid));

	}

	CStdioFile mod_file;
	CString mod_fname = this->m_edge_fname;
	mod_fname.Replace(_T(".txt"), _T(""));

	mod_fname += CString("_clean.txt");
	
	if (!mod_file.Open(mod_fname, CFile::modeWrite | CFile::modeCreate |CFile::typeText))
	{
		TRACE(_T("Unable to open file\n"));
	}

	int cntcnt = 0;
	for (auto & x : non_dup_edges)
	{
		CString strline;
		strline.Format(_T("%d\t%d\t%d\n"), cntcnt++, x.first.first, x.first.second);
		mod_file.WriteString(strline);
	}
...
/*
void CTNR::store_seg_pair_dist(pair<int,float> & qry_seg, pair<int, float> & obj_seg, float dist)
{
	//-----------------------------------------------------------
	//if (max(qry_seg.first, obj_seg.first) == 1634 && min(qry_seg.first, obj_seg.first) == 463)
	//{
	//	CString tt;
	//	tt.Format(_T("^^^^^ %d %f %d %f \n"), qry_seg.first, qry_seg.second, obj_seg.first, obj_seg.second);
	...
	m_seg_pair_dist.insert(make_pair(key, dist));
}
...
void CTNR::kFN_query(CQrySeg & qry_seg, const float qry_loc, CQry & qry, const vector<pair<int, pair<float, float>>>& sorted_obj_seg_list)
{
	g_num_kfn_queries++;
	...
	for (auto& item : sorted_obj_seg_list)
	{
		...
		CObjSeg& obj_seg = get_obj_seg_from_tbl(item.first);

		
		if (prundist > min_max_dist.second)
		{
			return;
		}

		CMySeq& obj_seq = this->get_seq_from_tbl(obj_segid);
		...
		for (auto& obj : obj_seq.obj_list)
		{
			...
			kth_dist = qry.update_result(obj_id, obj_dist);
...
void CTNR::MkFN_search(CMySeq & qry_seq, const vector<pair<int, pair<float, float>>> & sorted_obj_seg_list)
{
	...
	for (auto& q_id_loc : qry_seq.qry_list)
	{
		...
		CQry & qry = get_qry_from_tbl(qry_id);

		assert(qry.id == qry_id);



		kFN_query(qry_seg, qry_loc, qry, sorted_obj_seg_list);
